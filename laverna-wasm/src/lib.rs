...
pub fn evaluate(expression: &str) -> JsValue {
    let env = laverna::tanto::create_env();
    let result = match laverna::tanto::evaluate_expr(expression, &env) {
        Some(value) => {
...
pub fn verify(proof_json: &str) -> JsValue {
    ...
    let claimed = parsed.get("hash").and_then(|v| v.as_str()).unwrap_or("").to_string();
