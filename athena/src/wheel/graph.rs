//! # WheelGraph — the symbolic graph structure
//!
//! The graph is a 9-node directed graph where edges have aspects.
//! Nodes represent Vedic grahas (domains); edges represent relationships between them.
//! Traversal follows formula chains: from a starting graha, follow edges
//! to adjacent domains, collecting formulas along the way.
//!
//! # Performance
//!
//! Shortest paths between all 9×9 domain pairs are precomputed at construction
//! time using BFS, making `shortest_path()` an O(1) array lookup (~3ns) instead
//! of an on-demand BFS with heap allocations (~100ms).
...
/// domains at indices `a` and `b`.
///
/// On the 9-node Vedic wheel, edges exist at arc distances:
/// 0 (self/conjunction), 1 (sextile/adjacent), 3 (trine/trikona),
/// 4 (opposition/saptama). Distance 2 (square/varga) has no direct
/// edge but still has a meaningful aspect.
...
///
/// Maintains the 9-node graph structure and provides traversal methods.
/// Shortest paths between all domain pairs are precomputed at construction.
#[derive(Debug, Clone)]
pub struct WheelGraph {
    /// Adjacency list: domain → list of (neighbor, aspect) pairs
    adjacency: HashMap<Domain, Vec<(Domain, Aspect)>>,
    /// Precomputed shortest paths between every domain pair.
    /// Indexed by `[from.index()][to.index()]`, computed once in `new()`.
...
impl WheelGraph {
    /// Create a new wheel graph with all 9 Vedic graha nodes and their natural connections.
    ///
    /// Nodes are connected by four aspect types on the 9-node wheel:
    /// - Sextile (1 step = 40°, adjacent)
    /// - Trine (3 steps = 120°, harmonious)
    /// - Opposition (4 steps = 160°, full aspect)
    /// - Conjunction (self-loop, for internal formula chaining)
    ///
    /// Square (2 steps = 80°) has an aspect relationship but no direct edge.
    ///
    /// Also precomputes shortest paths between all 9×9 domain pairs
    /// via BFS from each node (negligible cost for 9 nodes).
    pub fn new() -> Self {
        let mut adjacency: HashMap<Domain, Vec<(Domain, Aspect)>> = HashMap::new();

        for &domain in ALL_DOMAINS.iter() {
            let mut edges: Vec<(Domain, Aspect)> = Vec::new();

            // Conjunction (self)
            edges.push((domain, Aspect::Conjunction));

            // Sextile (adjacent — 1 step)
            for adj in domain.adjacent() {
                edges.push((adj, Aspect::Sextile));
            }

            // Trine (3 steps = 120°)
            for trine in domain.trines() {
                edges.push((trine, Aspect::Trine));
            }

            // Opposition (4 steps = 160° — closest to 180° on 9-node)
            edges.push((domain.offset(4), Aspect::Opposition));
    ...
    #[inline]
    pub fn neighbors(&self, domain: Domain, aspect_filter: Option<Aspect>) -> Vec<Domain> {
        self.adjacency
            .get(&domain)
            .map(|edges| {
                edges
    ...
    /// Find all paths between `from` and `to` with maximum depth `max_depth`.
    ///
    /// Uses BFS to find shortest paths, then extends to find all paths within
    /// twice the shortest distance.
    pub fn find_paths(
        &self,
        from: Domain,
        to: Domain,
    ...
    /// Find the shortest path between two domains.
    ///
    /// Uses the precomputed 9×9 shortest-path table for O(1) lookup.
    /// Returns all paths for callers that need them via `find_paths()`.
    ...
    /// Get the aspect between two domains.
    #[inline]
    pub fn aspect_between(&self, a: Domain, b: Domain) -> Aspect {
        Aspect::between(a, b)
    }

    /// Check if a direct edge exists between two domains.
    ///
    /// Uses a precomputed 9×9 table for O(1) lookup instead of
    /// HashMap lookup + linear scan over the adjacency list.
    ///
    /// On the 9-node Vedic wheel, edges exist at arc distances:
    /// 0 (self/conjunction), 1 (sextile/adjacent), 3 (trine/trikona),
    /// and 4 (opposition/saptama). Distance 2 (square/varga) is
    /// not directly connected but still has a meaningful aspect.
    #[inline]
...
mod tests {
    ...
    fn test_graph_has_adjacent_edges() {
        ...
        // Chandra (1) and Mangala (2) are adjacent
    ...
    fn test_graph_has_trines() {
        let g = WheelGraph::new();
        // Mangala (2) → Shukra (5) = 3 steps = trine
        assert!(g.has_edge(Domain::Mangala, Domain::Shukra));
        // Mangala (2) → Ketu (8) = 3 steps (wrapping) = trine
    ...
    fn test_shortest_path_adjacent() {
        let g = WheelGraph::new();
        // Surya (0) → Chandra (1) = direct edge (adjacent)
    ...
    fn test_shortest_path_opposite() {
        let g = WheelGraph::new();
        // Surya (0) → Brihaspati (4) = 4 steps = opposition (direct edge)
        let path = g.shortest_path(Domain::Surya, Domain::Brihaspati).unwrap();
        assert_eq!(path.len(), 2); // direct edge via opposition
        assert_eq!(path[0], Domain::Surya);
    ...
    fn test_shortest_path_complex() {
        let g = WheelGraph::new();
        // Mangala (2) to Budha (3): adjacent, should succeed
    ...
    fn test_neighbors_filtered() {
        let g = WheelGraph::new();
        // Mangala (2) trines: Shukra (5, offset 3) and Ketu (8, offset 6)
        let trine_neighbors = g.neighbors(Domain::Mangala, Some(Aspect::Trine));
